...
- [x] 5.2 Implement the manual-recheck button as a scheduler request that remains subject to deduplication and provider limits, and verify an integration test covers button invocation during a pending job.
- [x] 5.3 Add concise bounded session details, translated entity names/states, icon handling, and entity categories/default enablement consistent with Home Assistant guidance, and verify the integration tests do not expose raw provider payloads.
